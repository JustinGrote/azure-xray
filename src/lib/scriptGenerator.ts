...
function parseAzureUrl(url: string): ParsedAzureUrl {
    ...
    // Parse path
    let currentIndex = 0;
    if (pathParts[currentIndex] === 'subscriptions' && pathParts[currentIndex + 1]) {
        result.subscriptionId = pathParts[currentIndex + 1];
        currentIndex += 2;
    }

    if (pathParts[currentIndex] === 'resourceGroups' && pathParts[currentIndex + 1]) {
...
export function generatePowerShellScript(request: AzureApiRequest): string {
    const { commandName, ...filteredHeaders } = request.requestHeaderDetails;

    // Special handling for Resource Graph queries
    if (request.httpMethod === 'POST' && request.url.startsWith('/providers/Microsoft.ResourceGraph')) {
    ...
    const useParameterFormat = parsedUrl &&
        parsedUrl.apiVersion &&
        (request.httpMethod !== 'GET' || Object.keys(parsedUrl.otherParams || {}).length === 0);

    if (useParameterFormat && parsedUrl) {
        let script = '$armParams = @{\n';

        if (parsedUrl.subscriptionId) script += `${indent}SubscriptionId = '${parsedUrl.subscriptionId}'\n`;
        if (parsedUrl.resourceGroupName) script += `${indent}ResourceGroupName = '${parsedUrl.resourceGroupName}'\n`;
        if (parsedUrl.resourceProviderName) script += `${indent}ResourceProviderName = '${parsedUrl.resourceProviderName}'\n`;
        if (parsedUrl.resourceType) script += `${indent}ResourceType = '${parsedUrl.resourceType.join("'/'")}\n'`;
        if (parsedUrl.name) script += `${indent}Name = '${parsedUrl.name.join("'/'")}\n'`;
        script += `${indent}ApiVersion = '${parsedUrl.apiVersion}'\n`;
        if (request.httpMethod !== 'GET') script += `${indent}Method = '${request.httpMethod}'\n`;
        ...
        const params = {
            ...
            Payload: request.content ? JSON.stringify(request.content, null, 2) : null
        };
